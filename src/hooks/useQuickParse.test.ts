...
describe('useQuickParse', () => {
  it('parses #project and exposes effective + display name', () => {
    const { result } = renderHook(() => useQuickParse('Water #Garden', ctx))
  ...
  it('does not stamp a context from the lens — only an explicit override sets one', () => {
    const { result } = renderHook(() => useQuickParse('Call the plumber', ctx))
    expect(result.current.effectiveParsed.context).toBeUndefined()
    act(() => result.current.applyContext('family'))
    expect(result.current.effectiveParsed.context).toBe('family')
  })
  it('clearContext override nulls an explicitly-applied context', () => {
    const { result } = renderHook(() => useQuickParse('Buy milk', ctx))
    act(() => result.current.applyContext('family'))
...
describe('useQuickParse — implicit contact resolution', () => {
  ...
  it('suggests an implicit contact when no explicit syntax matched', () => {
    const { result } = renderHook(() => useQuickParse('Call Macmillan Guitars', ctx, resolver))
  ...
  it('explicit @mention wins — resolver is skipped', () => {
    ...
    const { result } = renderHook(() => useQuickParse('Call @jon about Macmillan Guitars', ctx2, resolver))
  ...
  it('acceptSuggestion applies a ghost-band suggestion', () => {
    ...
    const { result } = renderHook(() => useQuickParse('call macmillan', ctx, fuzzyResolver))
  ...
  it('without a resolver arg, behavior is unchanged (no suggestion field set)', () => {
    const { result } = renderHook(() => useQuickParse('Call Macmillan Guitars', ctx))
    expect(result.current.suggestion).toBeNull()
    expect(result.current.effectiveParsed.contactId).toBeUndefined()
  })
})
