...
import type { MealPlan, MealPlanEntry, Recipe } from '@/types/meal-planner'

// Wed 2026-08-05 — the anchor day for most cases below.
const WED = new Date(2026, 7, 5)
const THIS_WEEK = new Date(2026, 7, 2)  // Sun Aug 2
const NEXT_WEEK = new Date(2026, 7, 9)  // Sun Aug 9
const PREV_WEEK = new Date(2026, 6, 26) // Sun Jul 26
...
function plan(over: Partial<MealPlan> & { id: string; weekStart: Date; entries: MealPlanEntry[] }): MealPlan {
  return {
    userId: 'u1', startsOn: null, endsOn: null,
    createdAt: new Date(2026, 0, 1), updatedAt: new Date(2026, 0, 1),
    ...over,
  }
}
...
describe('buildMealDayRecipes', () => {
  ...
  it('returns one entry per day that has a cookable recipe, in date order', () => {
    const days = buildMealDayRecipes({
      plans: [plan({
        id: 'p1', weekStart: THIS_WEEK, entries: [
  ...
  it('crosses the week boundary using the neighbouring plans', () => {
    const days = buildMealDayRecipes({
      plans: [
        plan({ id: 'p0', weekStart: PREV_WEEK, entries: [entry({ id: 'e0', mealPlanId: 'p0', dayOfWeek: 5, recipeId: 'r1' })] }), // Fri Jul 31
        plan({ id: 'p2', weekStart: NEXT_WEEK, entries: [entry({ id: 'e3', mealPlanId: 'p2', dayOfWeek: 1, recipeId: 'r2' })] }), // Mon Aug 10
      ],
      recipes: [piccata, tacos],
      centerDate: WED,
      slot: 'dinner',
  ...
  it('keeps a body-less meal that still has a source URL to fetch', () => {
    ...
    const days = buildMealDayRecipes({
      plans: [plan({ id: 'p1', weekStart: THIS_WEEK, entries: [entry({ id: 'e1', dayOfWeek: 2, recipeId: 'r3' })] })],
      recipes: [linkOnly],
      centerDate: WED,
      slot: 'dinner',
    })
  ...
  it('breaks a duplicate-week tie the same way useMealPlan does — oldest plan wins', () => {
    const days = buildMealDayRecipes({
      plans: [
        plan({
          id: 'newer', weekStart: THIS_WEEK, createdAt: new Date(2026, 5, 2),
          entries: [entry({ id: 'e1', mealPlanId: 'newer', dayOfWeek: 2, recipeId: 'r2' })],
        }),
        plan({
          id: 'older', weekStart: THIS_WEEK, createdAt: new Date(2026, 5, 1),
  ...
  it('stays inside the radius', () => {
    const days = buildMealDayRecipes({
      plans: [plan({ id: 'p1', weekStart: THIS_WEEK, entries: [entry({ id: 'e1', dayOfWeek: 2, recipeId: 'r1' })] })],
      recipes: [piccata],
      centerDate: WED,
      slot: 'dinner',
      radiusDays: 0,
