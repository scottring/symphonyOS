...
export async function applySession(
  ...
  const periodStart = parseLocalYmd(d.periodStart)
  ...
  for (const [id, v] of ordered) {
    if (v === 'keep-action') {
      if (!cur.keptAlready.includes(id)) {
        if (!(await wrote(() => w.keep(id, periodStart, prevStart)))) continue
        progress({ ...cur, keptAlready: [...cur.keptAlready, id] })
      }
      const title = cur.actionTitles[id]?.trim()
      const actionId = cur.actionIds[id]
      if (title && actionId && !(await wrote(() => w.addTask(title, { id: actionId, periodStart, goalTaskId: id, context: w.contextOf(id) })))) continue
