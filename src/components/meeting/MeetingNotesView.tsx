...
import { ArrowLeft, Clock, Users, Save, Loader2, Phone, Mail, FileText, CheckSquare, User } from 'lucide-react'
        ...
        <div className="px-4 pb-4 space-y-3 border-t border-neutral-100">
          ...
          {contact && (
            <div className="pt-3 space-y-1.5">
              ...
              {contact.notes && (
                <div className="text-sm text-neutral-500 mt-2 pl-5.5">
                  {contact.notes}
                </div>
              )}
            </div>
          ...
          {/* No matched contact */}
          {!contact && (
            <div className="pt-3 text-sm text-neutral-400 italic">
              No matching contact found in Symphony
            </div>
          )}
            ...
            <div className="pt-2">
              ...
              <div className="space-y-1.5">
                {recentNotes.map((note) => (
                  <div
                    key={note.id}
                    className="text-sm text-neutral-600 bg-neutral-50 rounded-lg px-3 py-2 line-clamp-3"
                  >
                    {note.title && (
                      <div className="font-medium text-neutral-700 mb-0.5">{note.title}</div>
                    )}
                ...
                {recentTasks.map((task) => (
                  ...
                  >
                    <div className={`w-3.5 h-3.5 rounded border ${
                      task.completed ? 'bg-primary-500 border-primary-500' : 'border-neutral-300'
                    }`} />
          ...
          {contact && !hasContext && (
            <div className="pt-2 text-sm text-neutral-400 italic">
              No notes or tasks found for this contact
            </div>
          )}
        </div>
      )}
...
export function MeetingNotesView({ meeting, onSaveNote, onEndMeeting }: MeetingNotesViewProps) {
  ...
  // Sync content from meeting state when it changes externally (e.g., loaded from DB)
  useEffect(() => {
    if (meeting.noteContent !== localContent && !meeting.saving) {
      setLocalContent(meeting.noteContent)
    }
  // Only sync when noteContent changes from load, not from our own edits
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [meeting.noteId])

  // Auto-focus textarea
  useEffect(() => {
    if (!meeting.loading && textareaRef.current) {
