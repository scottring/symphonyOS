...
interface DetailPanelRedesignProps {
  ...
  // Parent task (for follow-up context lineage)
    ...
    <div className="h-full flex flex-col bg-bg-base">
      ...
      <div className="flex-1 overflow-auto min-h-0 scroll-container">
        ...
        <div className="p-6 space-y-6 border-b border-neutral-100">
                          ...
                          >
                            {subtask.phoneNumber && (
                              <div className="flex items-center gap-1.5 text-xs text-primary-600">
                                ...
                                <a
                                  href={`tel:${subtask.phoneNumber.replace(/[^\d+]/g, '')}`}
          ...
          {/* Project Context - Links & Phone from Project */}
          {project && (project.links?.length || project.phoneNumber) && (
            <div className="bg-amber-50/50 rounded-xl p-4 border border-amber-100">
              ...
              <div className="space-y-3">
                ...
                {project.phoneNumber && (
                  <div>
                    <div className="text-xs text-amber-600 font-medium mb-1">Phone</div>
                    <a
                      href={`tel:${project.phoneNumber}`}
                      className="flex items-center gap-2 text-sm text-primary-600 hover:text-primary-700 font-medium p-2 -mx-2 rounded-lg hover:bg-white transition-colors"
                    >
                ...
                {project.links && project.links.length > 0 && (
                  <div>
                    <div className="text-xs text-amber-600 font-medium mb-1">Links</div>
                    <div className="space-y-1">
                      {project.links.map((link, index) => (
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex items-center gap-2 text-sm text-primary-600 hover:text-primary-700 p-2 -mx-2 rounded-lg hover:bg-white transition-colors"
                        >
                ...
                {project.notes && (
                  <div>
                    <div className="text-xs text-amber-600 font-medium mb-1">Project Notes</div>
                    <div className="text-sm text-neutral-700 whitespace-pre-wrap p-2 -mx-2">
                      {project.notes}
                    </div>
                  </div>
                )}
          ...
          <div>
            ...
            {/* Inherited context from parent task (follow-up lineage) */}
            {parentTask && (
              <div className="mb-3">
                <button
                  onClick={() => onOpenParentTask?.(parentTask.id)}
                  className="text-xs text-primary-600 font-medium mb-2 flex items-center gap-1 hover:underline cursor-pointer"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" className="w-3 h-3" viewBox="0 0 20 20" fill="currentColor">
                    <path fillRule="evenodd" d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clipRule="evenodd" />
                  </svg>
                  Continues from: {parentTask.title}
                </button>
                {(parentTask.notes || parentTask.phoneNumber) && (
                  <div className="text-sm text-neutral-600 bg-neutral-50 rounded-lg p-3 border border-neutral-100 space-y-2">
                    {parentTask.phoneNumber && (
                      <div className="flex items-center gap-2">
                        <svg xmlns="http://www.w3.org/2000/svg" className="w-3.5 h-3.5 text-neutral-400 shrink-0" viewBox="0 0 20 20" fill="currentColor">
                          <path d="M2 3a1 1 0 011-1h2.153a1 1 0 01.986.836l.74 4.435a1 1 0 01-.54 1.06l-1.548.773a11.037 11.037 0 006.105 6.105l.774-1.548a1 1 0 011.059-.54l4.435.74a1 1 0 01.836.986V17a1 1 0 01-1 1h-2C7.82 18 2 12.18 2 5V3z" />
                        </svg>
                        <a
                          href={`tel:${parentTask.phoneNumber}`}
                          className="text-primary-600 hover:underline"
                          onClick={(e) => e.stopPropagation()}
                        >
                          {parentTask.phoneNumber}
                    ...
                    {parentTask.notes && (
                      <div className="text-neutral-500 text-xs whitespace-pre-wrap line-clamp-4">
                        {parentTask.notes.replace(/<[^>]*>/g, '')}
                      </div>
                    )}
                  </div>
                )}
          ...
          <div className="border-b border-neutral-100">
            ...
            {detailsExpanded && (
              <div className="px-6 pb-6 space-y-1">
                ...
                {/* Context Row */}
                {item.originalTask && onUpdate && (
                  <div
                    className="flex items-center gap-3 p-3 -mx-3 rounded-lg"
                    onClick={(e) => e.stopPropagation()}
                  >
                    ...
                    <span className="text-sm text-neutral-500 w-20">Context</span>
                    <ContextPicker
                      value={item.originalTask.context}
                      onChange={(context) => onUpdate(item.originalTask!.id, { context })}
                    />
                  </div>
                )}
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <PlacesAutocomplete
                  ...
                  onSelect={(place: PlaceSelection) => {
                    // Must use google_event_id, not the internal UUID
