...
describe('WeekPlanHost', () => {
  ...
  it('says the week is not planned, opens the session in place of the days, and saves every decision once', async () => {
    ...
    expect(screen.getByRole('button', { name: /save this week/i })).toBeDisabled()
    fireEvent.change(screen.getByLabelText('Domain for Three bids'), { target: { value: 'family' } })
    ...
    expect(hook.updateTask).toHaveBeenCalledWith('m', { bucket: 'week', weekStart: WEEK, context: 'family' })
