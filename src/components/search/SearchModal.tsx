...
export function SearchModal({
  ...
  const flatResults = useCallback((): SearchResult[] => {
    ...
    if (results.notes.length > 0) addResults(results.notes, 'notes')
  ...
  const getResultIndex = (type: string, indexInType: number): number => {
    ...
    offset += Math.min(
      ...
      expandedSections.has('lists') ? results.lists.length : MAX_RESULTS_PER_TYPE
    )

    return offset + indexInType // notes
  }
          ...
          <div className="max-h-[40vh] overflow-y-auto rounded-xl border border-neutral-200 bg-neutral-50">
            {isSearching ? (
              ...
              <div className="p-2">
                ...
                {renderSection('Notes', results.notes, 'notes')}
              </div>
            )}
          </div>
        )}
