    ...
    <div className="relative bg-white/85 border border-stone-200/70 rounded-2xl p-4 overflow-hidden">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="leading-tight">
          <div className="font-display text-[2.1rem] text-stone-800 leading-none">
            {Math.round(data.temp)}°
          </div>
          <div className="text-[0.85rem] text-stone-500">{data.condition}</div>
        </div>
      </div>
      <div className="mt-2.5 text-[0.85rem] text-stone-600">
        Rain chance {data.rainChance}%
      </div>
      {data.sentence && (
        <div className="mt-0.5 text-[0.85rem] text-stone-500">
          {data.sentence}
        </div>
      )}
      <Leaf className="absolute bottom-2 right-2 w-12 h-12 text-emerald-200/80 -rotate-12" aria-hidden />
    </div>
  );
}
