...
export function EVRoutePlanner({ event, onClose, onSelectWaypoints }: EVRoutePlannerProps) {
  ...
  const handleResultHover = useCallback((placeId: string) => {
  ...
  const handleResultSelect = useCallback((prediction: google.maps.places.AutocompletePrediction) => {
    alert(`Add "${prediction.description}" as a waypoint in your trip, then recalculate the route.`)
    setSearchQuery('')
    setSearchResults([])
    setShowSearchResults(false)
    handleResultLeave()
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <aside className="w-96 flex-shrink-0 bg-neutral-50 border-l border-neutral-200 flex flex-col overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4">
              {/* Manual Entry */}
              <div className="mb-4 p-4 bg-amber-50 border border-amber-200 rounded-xl relative">
                <h3 className="text-sm font-semibold text-amber-900 mb-2">Add Manual Charging Stop</h3>
                <p className="text-xs text-amber-700 mb-3">Can't find the station you want? Search for any location:</p>
                <input
                  type="text"
                  value={searchQuery}
                  onChange={(e) => handleSearchInput(e.target.value)}
                  placeholder="Search for charging station (e.g., 'Electrify America Cherry Hill')"
                ...
                {showSearchResults && searchResults.length > 0 && (
                  <div className="absolute left-4 right-4 mt-1 bg-white border border-amber-300 rounded-lg shadow-lg max-h-64 overflow-y-auto z-50">
                    {searchResults.map((result) => (
                      <div
                        key={result.place_id}
                        className="px-4 py-3 hover:bg-amber-50 cursor-pointer border-b border-neutral-100 last:border-b-0 transition-colors"
                        onClick={() => handleResultSelect(result)}
                        onMouseEnter={() => handleResultHover(result.place_id)}
                        onMouseLeave={handleResultLeave}
                      >
                        <div className="text-sm font-medium text-neutral-900">{result.structured_formatting.main_text}</div>
                        <div className="text-xs text-neutral-600 mt-0.5">{result.structured_formatting.secondary_text}</div>
                      </div>
                    ))}
                  </div>
                )}

                <p className="text-xs text-amber-600 mt-2">Hover over results to see location on map. Click to select.</p>
              </div>
