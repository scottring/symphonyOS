...
export function MoreSheet({ isOpen, onClose, onNavigate, activeView, hasCheckinNudge }: MoreSheetProps) {
  const items: NavItem[] = [
    {
      view: 'routines',
      label: 'Routines',
      ...
      >
        ...
        <div className="grid grid-cols-3 gap-1 px-4 pb-4">
          {items.map(item => (
            ...
            >
              {item.badge && (
                <span className="absolute top-3 right-[calc(50%-4px)] w-2 h-2 bg-amber-400 rounded-full" />
              )}
              {item.icon}
