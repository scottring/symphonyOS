...
interface PromoteToProjectButtonProps {
  item: TimelineItem
  /** Whether this event appears multiple times (recurring) — shows a subtle indicator */
  isSuggestedPromotion?: boolean
}

/**
 * Button shown on calendar events to promote them to a project.
 * If the event already has a linked project, shows a "View Project" link instead.
 */
export function PromoteToProjectButton({ item, isSuggestedPromotion }: PromoteToProjectButtonProps) {
  ...
  const linkedProjectId = eventNote?.projectId || item.projectId

  // If event already has a linked project, show "View Project"
  if (linkedProjectId) {
    const project = ctx.projectsMap?.get(linkedProjectId)
    ...
    return (
      <button
        onClick={(e) => {
          e.stopPropagation()
          ctx.onOpenProject?.(linkedProjectId)
        }}
      ...
      <button
        ...
        className={`
          ...
          ${isSuggestedPromotion
            ? 'text-amber-400 hover:text-amber-600 hover:bg-amber-50 opacity-60 group-hover:opacity-100'
            : 'text-neutral-400 hover:text-primary-600 hover:bg-primary-50 opacity-0 group-hover:opacity-100'
          }
        `}
        title="Create project from event"
        aria-label="Create project from event"
      >
...
interface PromoteToProjectModalProps {
  item: TimelineItem
  onClose: () => void
}
...
export function PromoteToProjectModal({ item, onClose }: PromoteToProjectModalProps) {
  ...
  const [notes, setNotes] = useState(() => {
    ...
    if (item.googleDescription) {
      parts.push(item.googleDescription)
    }
  ...
  const handleSubmit = useCallback(async () => {
    ...
    const newProject: Project | null = await ctx.onAddProject?.({
    ...
    // 2. Link the event to the new project
    const eventId = item.originalEvent?.google_event_id || item.originalEvent?.id || item.id.replace('event-', '')
    const eventStartTime = item.startTime ?? undefined

    await ctx.onUpdateEventProject?.(
      eventId,
      newProject.id,
      item.title,
      eventStartTime,
    ...
    setSubmitting(false)
    onClose()
  }, [projectName, notes, context, submitting, ctx, item, onClose])
        ...
        <div className="bg-primary-50 border-b border-primary-100 px-5 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-neutral-500">from calendar event</p>
            </div>
          </div>
        </div>
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-4 py-3 text-sm rounded-xl border border-neutral-200 resize-none
                         focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent"
              placeholder="Event description, attendees, etc."
            />
          </div>
        </div>
