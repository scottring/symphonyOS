...
export function RescheduleGrid({ onPick, onPickDate, loads, onPeek, whens, flexibleWeekend = false }: Props) {
  const tiles = whens ? WHENS.filter((w) => whens.includes(w.when)) : WHENS
