...
describe('TimelineQuickInput', () => {
  it('shows kind + anchor time in the placeholder', () => {
    render(<TimelineQuickInput kind="task" anchorTime={anchor} parserContext={pc} onSubmit={vi.fn()} onCancel={vi.fn()} />)
  ...
  it('Enter with text submits effective result; no typed time → scheduledFor = anchor', () => {
    ...
    render(<TimelineQuickInput kind="task" anchorTime={anchor} parserContext={pc} onSubmit={onSubmit} onCancel={vi.fn()} />)
  ...
  it('a typed date overrides the anchor time', () => {
    const onSubmit = vi.fn()
    render(<TimelineQuickInput kind="task" anchorTime={new Date(2026,4,19,18,15)} parserContext={{ projects:[], contacts:[], familyMembers:[] }} onSubmit={onSubmit} onCancel={vi.fn()} />)
    const inp = screen.getByPlaceholderText(/new task ·/i)
  ...
  it('empty Enter does nothing; Esc cancels', () => {
    ...
    render(<TimelineQuickInput kind="event" anchorTime={anchor} parserContext={pc} onSubmit={onSubmit} onCancel={onCancel} />)
