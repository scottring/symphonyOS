#!/usr/bin/env bash
# Investigation: what a review Drop leaves in the database when one of its
# requests fails, and how candidate fixes behave. Runs
# supabase/tests/097_drop_partial_failure.investigation.sql against a
# throwaway local Postgres 17 loaded with the LIVE schema fixture
# (supabase/tests/fixtures/planning_commitments_schema.sql). Write-up:
# docs/planning/2026-09-25-drop-partial-failure-investigation.md
#
#   ./scripts/investigate-drop-partial-failure.sh          # PASS / FAIL + OBSERVE lines
...
# fidelity gaps as scripts/test-planning-commitments-locally.sh (no PostgREST;
...
OUT="$(psql -f supabase/tests/097_drop_partial_failure.investigation.sql 2>&1 || true)"
if [ -n "${VERBOSE:-}" ]; then printf '%s\n' "$OUT"; fi
printf '%s\n' "$OUT" | sed -n 's/.*NOTICE: *\(OBSERVE .*\)/\1/p; s/.*NOTICE: *\(PASS .*\)/\1/p; s/.*ERROR: *\(.*\)/FAIL-> \1/p'
