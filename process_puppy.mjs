...
async function processSprite() {
    try {
        ...
        const q1 = img.clone().crop({ x: 0, y: 0, w: crop_w, h: crop_h });
        const q2 = img.clone().crop({ x: w_mid, y: 0, w: crop_w, h: crop_h });
        const q3 = img.clone().crop({ x: 0, y: h_mid, w: crop_w, h: crop_h });
        const q4 = img.clone().crop({ x: w_mid, y: h_mid, w: crop_w, h: crop_h });

        // Create a new wide image to hold the 4 frames horizontally
        const finalImg = new Jimp({ width: crop_w * 4, height: crop_h, color: 0x00000000 });
        ...
        finalImg.scan(0, 0, finalImg.bitmap.width, finalImg.bitmap.height, function (x, y, idx) {
        ...
        const targetWidth = Math.floor((crop_w * 4) * (targetHeight / crop_h));
        finalImg.resize({ w: targetWidth, h: targetHeight });

        await finalImg.write('/Users/scottkaufman/Developer/Developer/symphonyOS/public/puppy_sprite.png');
