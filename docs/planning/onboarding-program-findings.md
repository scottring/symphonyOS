...
| S1-15 | 1 | usability | medium | Sit on a planning page and look at the two rows. | The horizon row reads as belonging to Planner. | It reads as a second, unrelated row. Scott: "we need some visual indication of the fact that the today - week-month... row is a subrow of Planner". Both rows already share the same 56px left edge, so alignment carried nothing. **Fixed:** Planner is underlined on a planning page and a hairline drops from that underline into the band (`index.css`, desktop only — the phone uses `HorizonSwitcher`). A notch in the band's own `neutral-100` was tried first and was invisible against the warm page ground. | Live walk 2026-09-23; screenshot | fixed |
