 ...
 *
 * Full coverage (flag a family-domain task → see widget on /wall-v2 → mark as discussed)
 ...
 *   4. Navigate to /wall-v2 and verify the kiosk widget renders with the task.
...
test.describe('Needs discussion (smoke)', () => {
  test('the /wall-v2 route responds without crashing', async ({ page }) => {
    const response = await page.goto('/wall-v2')
