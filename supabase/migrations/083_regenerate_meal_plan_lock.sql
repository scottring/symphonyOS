-- 083_regenerate_meal_plan_lock.sql
-- Add a row lock on meal_plans inside regenerate_meal_plan to serialize
-- concurrent generation attempts on the same week. Without this, two callers
-- can both read prior state simultaneously, both insert, and produce
-- duplicated/orphaned entries. The lock blocks the second caller until the
-- first commits.
...
declare
  ...
  v_owner_id uuid;
begin
  -- Lock the meal_plans row first so concurrent calls serialize.
  ...
  if not (v_owner_id = auth.uid() or users_share_household(auth.uid(), v_owner_id)) then
