...
serve(async (req) => {
  ...
  try {
    ...
    const body: MoveEventRequest = await req.json()
    const { eventId, sourceCalendarId, destinationCalendarId } = body

    if (!eventId || !sourceCalendarId || !destinationCalendarId) {
      return new Response(JSON.stringify({
        error: 'Missing required fields: eventId, sourceCalendarId, destinationCalendarId',
      }), {
        status: 200,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    ...
    // Google Calendar events.move endpoint
    // POST /calendars/{calendarId}/events/{eventId}/move?destination={destinationId}
    ...
    if (!moveResponse.ok) {
      ...
      console.error('Google Calendar move API error:', moveResponse.status, errorText)
      ...
      return new Response(JSON.stringify({
        error: errorMessage,
        statusCode: moveResponse.status,
      }), {
        status: 200,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    ...
    return new Response(JSON.stringify({
      ...
      htmlLink: moveData.htmlLink,
    }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    })
  } catch (error) {
    console.error('Error in google-calendar-move-event:', error)
