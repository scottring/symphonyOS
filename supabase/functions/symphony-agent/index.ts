...
async function callAnthropic(
...
): Promise<{ content: AnthropicBlock[]; stop_reason: string; containerId: string | null }> {
  // Symphony's own tools first (minus the destructive ones once the web has
  // been read), then Anthropic's server-side web tools. The cache breakpoint
  // rides the last entry, so it must be computed after the concat — it has to
  // cover the whole list, whatever the fence removed.
  const symphonyTools = toolsForTurn(TOOLS, webTouched)
  const tools: unknown[] = [...symphonyTools, ...WEB_TOOLS]
  const res = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({
      ...
      messages,
      // The _20260209 web tools do their dynamic filtering by running code
      // server-side, which provisions a container. Every follow-up request in
      // the same turn must name it, or Anthropic 400s with "container_id is
      // required when there are pending tool uses generated by code execution
      // with tools" the moment a Symphony tool call follows a web fetch.
...
Deno.serve(async (req) => {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      // Latches once the web enters this turn; from then on callAnthropic
      // withholds the destructive tools. Never reset — a page read early in
      // a turn is still in context at the end of it.
      let webTouched = false
      // Set once a web tool provisions a server-side container; carried on
      // every later request in this turn. Sticky — a container survives the
      // turn, and dropping it mid-turn is what triggers the 400.
      let containerId: string | null = null
      try {
        for (let turn = 0; turn < MAX_TURNS; turn++) {
          const res = await callAnthropic(apiKey, convo, webTouched, containerId)
          const { content, stop_reason } = res
          if (res.containerId) containerId = res.containerId
          const toolResults: Array<Record<string, unknown>> = []
          for (const block of content) {
            if (block.type === 'text' && block.text) {
              // Accumulate, don't overwrite: a cited answer arrives as SEVERAL
              // text blocks in one response, and `reply` on the done event has
              // to carry all of them or the pane keeps only the last fragment.
              finalText += block.text
              send({ type: 'text', text: block.text })
            } else if (isWebBlock(block)) {
              // Anthropic already ran this one — surface it so the pane says
              // "web_search" instead of stalling silently, and drop the fence.
              if (!webTouched) webTouched = true
          ...
          // A server tool that runs long pauses the turn. Re-send with the
          // assistant turn appended and NO tool results — Anthropic resumes
          // where it left off. Without this the answer truncates in silence.
